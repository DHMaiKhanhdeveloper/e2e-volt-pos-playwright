 ...
 *   TC-3  Card description text — Total Order
 *   TC-5  Card description text — Sale
 *   TC-7  Card description text — Total Tip
 *   TC-9  Card description text — Total Payment
 ...
 *
 * Note: each card's helper text is an always-visible `<p>` under the
 * heading, NOT a hover tooltip — there is no ⓘ info button in Volt POS.
 */
...
test.describe(`Daily Sale Report — defaults & card descriptions ${Tag.REGRESSION}`, () => {
  ...
  for (const [card, expected] of Object.entries(CARD_DESCRIPTIONS) as Array<[ChartCard, string]>) {
    test(`TC-3/5/7/9: card description — ${card}`, async ({ dailySaleReportPage }) => {
      // The description is always-visible helper text under the heading.
      // Compare normalised whitespace so a stray double-space won't flake.
      const actual = ((await dailySaleReportPage.cardDescription(card).textContent()) ?? '')
        .replace(/\s+/g, ' ')
        .trim();
